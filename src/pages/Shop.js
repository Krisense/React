      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {SHOP_ITEMS.map(item => (
            <div 
              key={item.id} 
              className={`border rounded-lg p-4 ${level < item.unlockLevel ? 'opacity-50' : ''}`}
            >
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                {level >= item.unlockLevel ? (
                  ...
                  <span className="text-sm text-gray-500">Доступно с {item.unlockLevel} уровня</span>
                )}
              </div>
            </div>
          ))}
